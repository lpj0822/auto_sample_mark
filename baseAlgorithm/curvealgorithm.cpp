...
bool CurveAlgorithm::reversePolynomialCurveFit(const QList<QPoint> &pointList, int n, cv::Mat& A)
{
    //Number of key points
    int count = pointList.count();
    ...
    for (int i = 0; i < n + 1; i++)
    {
        ...
        {
            Y.at<double>(i, 0) = Y.at<double>(i, 0) +
                    std::pow(pointList[k].y(), i) * pointList[k].x();
        }
    }
